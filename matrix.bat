...
echo. 
echo -----------------------
echo SATLINK ESTABLISHED
echo -----------------------
echo.
echo -----------------------
echo Choose login name:
echo -----------------------
set /p input= LOGIN NAME:
if %input%== alekso56 goto SATLINK
goto errorlog
:SATLINK
color 07
cls
echo CERN DATABASE Account Log In
echo ================================
echo.
...
echo.
echo 0) SECRET MENU
echo 1) User Control Panel
echo 2) Net Send Messenger
echo 3) Program Launcher
echo 4) Notepad
echo 5) Pop-up Maker
echo 6) Folder Factory
echo 7) Remote Shutdown (For computer networks)
echo 8) Return to normal menu?
echo.
set /p opcao=
if %opcao%==0 goto smenu
if %opcao%==1 goto aco1
if %opcao%==2 goto netmenu
if %opcao%==3 goto programs
if %opcao%==4 goto aco4
if %opcao%==5 goto aco5
if %opcao%==6 goto aco6
if %opcao%==7 goto aco7
if %opcao%==8 goto 3
goto erroraco
:erroraco
cls
...
echo -------------------------------------------------------------------------------
echo.
echo 1) Change Interface Colors
...
echo.
set /p chgpscon=Enter current password for this account:
...
echo cd "%userprofile%\documents\cmdacoBin" >> "ucp5.bat"
...
echo -------------------------------------------------------------------------------
echo.
echo Enter computername to send message to
echo.
...
echo.
set /p comm=Message:
if "%comm%"=="%comm%" goto aco2next2
:aco2next2
echo.
set /p spamop=Spam Message?(Y/N):
...
:makespam
cd "%userprofile%\documents\cmdacoBin"
echo @echo off > "netspam.bat"
echo call "Cmd Accounts.bat"
echo :spam >> "netspam.bat"
echo net send "%comname%" "%comm%" >> "netspam.bat"
echo goto spam >> "netspam.bat"
echo.
echo Spamming Message...
ping localhost -n 2 >nul
cd "%userprofile%\documents\cmdacoBin"
start netspam.bat
cls
...
echo -------------------------------------------------------------------------------
echo.
echo Message has been spammed to %comname%
echo.
pause
goto account
:programs
...
if %var%==57 start "%userprofile%\My Documents"
...
goto account
:aco3.3
if exist "%userprofile%\documents\cmdacoBin\myprograms%logname%" goto skip111
if not exist "%userprofile%\documents\cmdacoBin\myprograms%logname%" goto noskip111
:noskip111
cd "%userprofile%\documents\cmdacoBin"
...
goto addfiletolist
:addfiletolist
echo echo %cfne%                                %cnfcf% >> "%userprofile%\documents\cmdacoBin\myprograms%logname%.bat"
echo.
...
echo ------------------------------------------------------------------------------- 
echo.
echo %newfold% folder was created in your documents.
echo.
echo Press any key to return to your account. . .
pause >nul
goto account
...
echo. 
echo ------------------------------------------------------------------------------- 
echo %logname% Remote Shutdown  cockblocked
echo ------------------------------------------------------------------------------- 
echo.
echo Shutdown failure by faggot.exe
echo.
pause
goto matrix

:matrix
call c:/batch/matrix.bat
goto cont2